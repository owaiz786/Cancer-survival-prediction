    ...
    <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              {results.fileName ? `Batch Analysis: ${results.fileName}` : `Patient ID: ${results.patientId}`}
            </p>
          </div>
        </div>
      ...
      {!results.fileName ? (
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-gray-900">{results.totalPatients}</div>
              <p className="text-xs text-gray-600 mt-1">processed successfully</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">Average Survival</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-gray-900">{results.summary.averageSurvivalMonths.toFixed(1)}</div>
              <p className="text-xs text-gray-600 mt-1">months</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center space-x-2">
                <div className="text-2xl font-bold text-red-600">{results.summary.highRiskPatients}</div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-600">Model Performance</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-gray-900">
                {results.modelPerformance.deepsurv.cIndex.toFixed(3)}
              </div>
              <p className="text-xs text-gray-600 mt-1">best C-index (DeepSurv)</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      <Tabs defaultValue="overview" className="w-full">
                ...
                {!results.fileName ? (
                  ...
                  <>
                    <div className="flex items-start space-x-3">
                      <CheckCircle className="w-5 h-5 text-green-600 mt-0.5" />
                      <div>
                        <p className="font-medium text-sm">Batch Processing Complete</p>
                        <p className="text-sm text-gray-600">Successfully analyzed {results.totalPatients} patients</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">High Risk Patients</p>
                        <p className="text-sm text-gray-600">
                          {results.summary.highRiskPatients} patients require immediate clinical attention
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start space-x-3">
                      <TrendingUp className="w-5 h-5 text-blue-600 mt-0.5" />
                      <div>
                        <p className="font-medium text-sm">Population Insights</p>
                        <p className="text-sm text-gray-600">
                          Average survival: {results.summary.averageSurvivalMonths.toFixed(1)} months
                        </p>
                      </div>
                    </div>
                  </>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries(results.modelComparison || results.modelPerformance).map(
        ...
        <TabsContent value="features" className="space-y-6">
          <FeatureImportance features={results.featureImportance || results.topFeatures} />
        </TabsContent>
